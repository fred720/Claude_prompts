---
name: Create Image
allowed-tools: Bash, mcp_replicate_create_models_predictions, mcp_replicate_get_predictions
...
- First, check your system prompt to make sure you have `mcp_replicate_create_models_predictions` and `mcp_replicate_get_predictions` available. If not, STOP immediately and ask the user to add them.
...
- IMPORTANT: Then generate `NUMBER_OF_IMAGES` using the `IMAGE_GENERATION_PROMPT` following the `image-loop` below.
...
<image-loop>
    ...
    - Wait for completion by polling `mcp_replicate_get_predictions`
